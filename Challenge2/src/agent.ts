...
export function provideHandleTransaction(
  ...
  return async (txEvent: TransactionEvent) => {
    ...
    for (const filteredLog of filteredLogs) {
      ...
      const interceptedPoolValues = await getPoolValues(provider, uniPoolFunctionsAbi, interceptedPoolAddress, txEvent);
      ...
      findings.push(
        Finding.fromObject({
          ...
          description: `Address ${sender} swapped ${Math.abs(amount0)} of token ${interceptedPoolValues[0]} for ${amount1} of token ${interceptedPoolValues[1]}, using pool ${interceptedPoolAddress}`,
